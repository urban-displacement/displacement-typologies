...
exclusivity_measure <- function(state, counties) {
  ...
  tract_counts <- bind_rows(price_counts, value_counts) %>%
    ...
    mutate(high = (total - nonhigh)/total,
           nonhigh = nonhigh/total,
           low = low/total,
    ...
    mutate(high_ratio = high/high_income,
           ...
           nonhigh_access = nonhigh*nonhigh_income,
...
plot_exclusivity_access(puget, "nonhigh_access", "Access (<120% of AMI)")
...
plot_exclusivity_access(bay9, "nonhigh_access", "Access (<120% of AMI)")
