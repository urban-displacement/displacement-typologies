...
census <- readRDS(paste0('~/git/displacement-typologies/data/outputs/downloads/census_', states, '.rds'))
census_00 <- readRDS(paste0('~/git/displacement-typologies/data/outputs/downloads/census_00_', states, '.rds'))
census_90 <- readRDS(paste0('~/git/displacement-typologies/data/outputs/downloads/census_90_', states, '.rds'))
...
census_00_x <- 
	left_join(census_00, xwalk_00_10, by = c('GEOID' = 'trtid00')) %>% 
...
census_90_x <- 
	left_join(census_90, xwalk_90_10, by = c('GEOID' = 'trtid90')) %>% 
	mutate_at(vars(pop_90:I_201000_90), list(~ .*weight)) %>% 
	group_by(trtid10) %>% 
	summarize_at(vars(pop_90:I_201000_00), list(sum)) %>% 
	rename(GEOID = trtid10)
