...
df <- 
    bind_rows(
            read_csv("~/git/displacement-typologies/data/outputs/databases/Atlanta_database_2017.csv") %>% 
            select(!X1) %>% 
            mutate(city = "Atlanta"),
            read_csv("~/git/displacement-typologies/data/outputs/databases/Denver_database_2017.csv") %>% 
            select(!X1) %>% 
            mutate(city = "Denver"),
            read_csv("~/git/displacement-typologies/data/outputs/databases/Chicago_database_2017.csv") %>% 
            select(!X1) %>% 
            mutate(city = "Chicago"),
            read_csv("~/git/displacement-typologies/data/outputs/databases/Memphis_database_2017.csv") %>% 
            select(!X1) %>% 
            mutate(city = "Memphis")
    )
...
fwrite(lag, "~/git/displacement-typologies/data/outputs/lag_2017.csv")
